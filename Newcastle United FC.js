var request = new Request('http://api.football-data.org/v1/teams/67/players', {
	method: 'GET', 
	mode: 'cors', 
	redirect: 'follow',
	headers: new Headers({
...
var request2 = new Request('http://api.football-data.org/v1/teams/67/fixtures', {
	method: 'GET', 
	mode: 'cors', 
	redirect: 'follow',
	headers: new Headers({
...
logo.src="Team Logos/newcastle-united.png";
logo.setAttribute("style","margin-top:20");
logo.width=200;
var textnode=document.createTextNode("Newcastle United FC");
...
.then(function(data) {
	...
	fetch(request3)
			...
			.then(function(data) {
				...
				for(let i=0;i<20;i++)
					if(data["standing"][i]["teamName"]==="Newcastle United FC"){
	...
	.then(function(data) {
		var matchTab=document.createElement("table");
					...
					while(data["fixtures"][j]!==undefined){
						...
						if(data["fixtures"][j]["homeTeamName"]==="Newcastle United FC")
			...
			var coachName=document.createTextNode("Manager: Rafa Benitez");
			var coachImg=document.createElement("img");
			coachImg.src="Coaches/newcastle-united.jpg";
			...
			var stadiumName=document.createTextNode("Stadium: St James' Park");
			var stadImg=document.createElement("img");
			stadImg.src="Stadiums/newcastle-united.jpg";
