var request = new Request('http://api.football-data.org/v1/teams/354/players', {
	method: 'GET', 
	mode: 'cors', 
	redirect: 'follow',
	headers: new Headers({
...
var request2 = new Request('http://api.football-data.org/v1/teams/354/fixtures', {
	method: 'GET', 
	mode: 'cors', 
	redirect: 'follow',
	headers: new Headers({
...
logo.src="Team Logos/crystal-palace.png";
logo.setAttribute("style","margin-top:20");
logo.width=200;
var textnode=document.createTextNode("Crystal Palace FC");
...
.then(function(data) {
	...
	fetch(request3)
			...
			.then(function(data) {
				...
				for(let i=0;i<20;i++)
					if(data["standing"][i]["teamName"]==="Crystal Palace FC"){
	...
	.then(function(data) {
		var matchTab=document.createElement("table");
					...
					while(data["fixtures"][j]!==undefined){
						...
						if(data["fixtures"][j]["homeTeamName"]==="Crystal Palace FC")
			...
			var coachName=document.createTextNode("Manager: Roy Hodgson");
			var coachImg=document.createElement("img");
			coachImg.src="Coaches/crystal-palace.jpg";
			...
			var stadiumName=document.createTextNode("Stadium: Selhurst Park");
			var stadImg=document.createElement("img");
			stadImg.src="Stadiums/crystal-palace.jpg";
