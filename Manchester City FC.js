var request = new Request('http://api.football-data.org/v1/teams/65/players', {
	method: 'GET', 
	mode: 'cors', 
	redirect: 'follow',
	headers: new Headers({
...
var request2 = new Request('http://api.football-data.org/v1/teams/65/fixtures', {
	method: 'GET', 
	mode: 'cors', 
	redirect: 'follow',
	headers: new Headers({
...
logo.src="Team Logos/manchester-city.png";
logo.setAttribute("style","margin-top:20");
var textnode=document.createTextNode("Manchester City FC");
...
.then(function(data) {
	...
	fetch(request3)
			...
			.then(function(data) {
				...
				for(let i=0;i<20;i++)
					if(data["standing"][i]["teamName"]==="Manchester City FC"){
	...
	.then(function(data) {
		var matchTab=document.createElement("table");
					...
					while(data["fixtures"][j]!==undefined){
						...
						if(data["fixtures"][j]["homeTeamName"]==="Manchester City FC")
			...
			var coachName=document.createTextNode("Manager: Pep Guardiola");
			var coachImg=document.createElement("img");
			coachImg.src="Coaches/manchester-city.jpg";
			...
			var stadiumName=document.createTextNode("Stadium: Etihad Stadium");
			var stadImg=document.createElement("img");
			stadImg.src="Stadiums/manchester-city.jpg";
