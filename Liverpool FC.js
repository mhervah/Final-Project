var request = new Request('http://api.football-data.org/v1/teams/64/players', {
	method: 'GET', 
	mode: 'cors', 
	redirect: 'follow',
	headers: new Headers({
...
var request2 = new Request('http://api.football-data.org/v1/teams/64/fixtures', {
	method: 'GET', 
	mode: 'cors', 
	redirect: 'follow',
	headers: new Headers({
...
logo.src="Team Logos/liverpool.png";
logo.setAttribute("style","margin-top:20");
logo.width=200;
var textnode=document.createTextNode("Liverpool FC");
...
.then(function(data) {
	...
	fetch(request3)
			...
			.then(function(data) {
				...
				for(let i=0;i<20;i++)
					if(data["standing"][i]["teamName"]==="Liverpool FC"){
	...
	.then(function(data) {
		var matchTab=document.createElement("table");
					...
					while(data["fixtures"][j]!==undefined){
						...
						if(data["fixtures"][j]["homeTeamName"]==="Liverpool FC")
			...
			var coachName=document.createTextNode("Manager: Jurgen Klopp");
			var coachImg=document.createElement("img");
			coachImg.src="Coaches/liverpool.jpg";
			...
			var stadiumName=document.createTextNode("Stadium: Anfield");
			var stadImg=document.createElement("img");
			stadImg.src="Stadiums/liverpool.jpg";
