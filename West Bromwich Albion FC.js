var request = new Request('http://api.football-data.org/v1/teams/74/players', {
	method: 'GET', 
	mode: 'cors', 
	redirect: 'follow',
	headers: new Headers({
...
var request2 = new Request('http://api.football-data.org/v1/teams/74/fixtures', {
	method: 'GET', 
	mode: 'cors', 
	redirect: 'follow',
	headers: new Headers({
...
logo.src="Team Logos/west-brom.png";
logo.setAttribute("style","margin-top:20");
logo.width=200;
var textnode=document.createTextNode("West Bromwich Albion FC");
...
.then(function(data) {
	...
	fetch(request3)
			...
			.then(function(data) {
				...
				for(let i=0;i<20;i++)
					if(data["standing"][i]["teamName"]==="West Bromwich Albion FC"){
	...
	.then(function(data) {
		var matchTab=document.createElement("table");
					...
					while(data["fixtures"][j]!==undefined){
						...
						if(data["fixtures"][j]["homeTeamName"]==="West Bromwich Albion FC")
			...
			var coachName=document.createTextNode("Manager: Alan Pardew");
			var coachImg=document.createElement("img");
			coachImg.src="Coaches/west-brom.jpg";
			...
			var stadiumName=document.createTextNode("Stadium: The Hawthorns");
			var stadImg=document.createElement("img");
			stadImg.src="Stadiums/west-brom.jpg";
