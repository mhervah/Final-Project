var request = new Request('http://api.football-data.org/v1/teams/66/players', {
	method: 'GET', 
	mode: 'cors', 
	redirect: 'follow',
	headers: new Headers({
...
var request2 = new Request('http://api.football-data.org/v1/teams/66/fixtures', {
	method: 'GET', 
	mode: 'cors', 
	redirect: 'follow',
	headers: new Headers({
...
logo.src="Team Logos/manchester-united.png";
logo.setAttribute("style","margin-top:20");
logo.width=200;
var textnode=document.createTextNode("Manchester United FC");
...
.then(function(data) {
	...
	fetch(request3)
			...
			.then(function(data) {
				...
				for(let i=0;i<20;i++)
					if(data["standing"][i]["teamName"]==="Manchester United FC"){
	...
	.then(function(data) {
		var matchTab=document.createElement("table");
					...
					while(data["fixtures"][j]!==undefined){
						...
						if(data["fixtures"][j]["homeTeamName"]==="Manchester United FC")
			...
			var coachName=document.createTextNode("Manager: Jose Mourinho");
			var coachImg=document.createElement("img");
			coachImg.src="Coaches/manchester-united.jpg";
			...
			var stadiumName=document.createTextNode("Stadium: Old Trafford");
			var stadImg=document.createElement("img");
			stadImg.src="Stadiums/manchester-united.jpg";
