var request = new Request('http://api.football-data.org/v1/teams/70/players', {
	method: 'GET', 
	mode: 'cors', 
	redirect: 'follow',
	headers: new Headers({
...
var request2 = new Request('http://api.football-data.org/v1/teams/70/fixtures', {
	method: 'GET', 
	mode: 'cors', 
	redirect: 'follow',
	headers: new Headers({
...
logo.src="Team Logos/stoke-city.png";
logo.setAttribute("style","margin-top:20");
logo.width=200;
var textnode=document.createTextNode("Stoke City FC");
...
.then(function(data) {
	...
	fetch(request3)
			...
			.then(function(data) {
				...
				for(let i=0;i<20;i++)
					if(data["standing"][i]["teamName"]==="Stoke City FC"){
	...
	.then(function(data) {
		var matchTab=document.createElement("table");
					...
					while(data["fixtures"][j]!==undefined){
						...
						if(data["fixtures"][j]["homeTeamName"]==="Stoke City FC")
			...
			var coachName=document.createTextNode("Manager: Mark Hughes");
			var coachImg=document.createElement("img");
			coachImg.src="Coaches/stoke-city.jpg";
			...
			var stadiumName=document.createTextNode("Stadium: Britannia Stadium");
			var stadImg=document.createElement("img");
			stadImg.src="Stadiums/stoke-city.jpg";
