var request = new Request('http://api.football-data.org/v1/teams/394/players', {
	method: 'GET', 
	mode: 'cors', 
	redirect: 'follow',
	headers: new Headers({
...
var request2 = new Request('http://api.football-data.org/v1/teams/394/fixtures', {
	method: 'GET', 
	mode: 'cors', 
	redirect: 'follow',
	headers: new Headers({
...
logo.src="Team Logos/huddersfield.png";
logo.setAttribute("style","margin-top:20");
logo.width=200;
var textnode=document.createTextNode("Huddersfield Town");
...
.then(function(data) {
	...
	fetch(request3)
			...
			.then(function(data) {
				...
				for(let i=0;i<20;i++)
					if(data["standing"][i]["teamName"]==="Huddersfield Town"){
	...
	.then(function(data) {
		var matchTab=document.createElement("table");
					...
					while(data["fixtures"][j]!==undefined){
						...
						if(data["fixtures"][j]["homeTeamName"]==="Huddersfield Town")
			...
			var coachName=document.createTextNode("Manager: David Wagner");
			var coachImg=document.createElement("img");
			coachImg.src="Coaches/huddersfield.jpg";
			...
			var stadiumName=document.createTextNode("Stadium: John Smith's Stadium");
			var stadImg=document.createElement("img");
			stadImg.src="Stadiums/huddersfield.jpg";
