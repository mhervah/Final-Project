var request = new Request('http://api.football-data.org/v1/teams/346/players', {
	method: 'GET', 
	mode: 'cors', 
	redirect: 'follow',
	headers: new Headers({
...
var request2 = new Request('http://api.football-data.org/v1/teams/346/fixtures', {
	method: 'GET', 
	mode: 'cors', 
	redirect: 'follow',
	headers: new Headers({
...
logo.src="Team Logos/watford.png";
logo.setAttribute("style","margin-top:20");
logo.width=200;
var textnode=document.createTextNode("Watford FC");
...
.then(function(data) {
	...
	fetch(request3)
			...
			.then(function(data) {
				...
				for(let i=0;i<20;i++)
					if(data["standing"][i]["teamName"]==="Watford FC"){
	...
	.then(function(data) {
		var matchTab=document.createElement("table");
					...
					while(data["fixtures"][j]!==undefined){
						...
						if(data["fixtures"][j]["homeTeamName"]==="Watford FC")
			...
			var coachName=document.createTextNode("Manager: Marco Silva");
			var coachImg=document.createElement("img");
			coachImg.src="Coaches/watford.jpg";
			...
			var stadiumName=document.createTextNode("Stadium: Vicarage Road");
			var stadImg=document.createElement("img");
			stadImg.src="Stadiums/watford.jpg";
