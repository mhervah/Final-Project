var request = new Request('http://api.football-data.org/v1/teams/62/players', {
	method: 'GET', 
	mode: 'cors', 
	redirect: 'follow',
	headers: new Headers({
...
var request2 = new Request('http://api.football-data.org/v1/teams/62/fixtures', {
	method: 'GET', 
	mode: 'cors', 
	redirect: 'follow',
	headers: new Headers({
...
logo.src="Team Logos/everton.png";
logo.setAttribute("style","margin-top:20");
logo.width=200;
var textnode=document.createTextNode("Everton FC");
...
.then(function(data) {
	...
	fetch(request3)
			...
			.then(function(data) {
				...
				for(let i=0;i<20;i++)
					if(data["standing"][i]["teamName"]==="Everton FC"){
	...
	.then(function(data) {
		var matchTab=document.createElement("table");
					...
					while(data["fixtures"][j]!==undefined){
						...
						if(data["fixtures"][j]["homeTeamName"]==="Everton FC")
			...
			var coachName=document.createTextNode("Manager: Sam Allardyce");
			var coachImg=document.createElement("img");
			coachImg.src="Coaches/everton.jpg";
			...
			var stadiumName=document.createTextNode("Stadium: Goodison Park");
			var stadImg=document.createElement("img");
			stadImg.src="Stadiums/everton.jpg";
